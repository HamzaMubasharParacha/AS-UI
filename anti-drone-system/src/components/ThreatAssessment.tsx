...
const ThreatAssessment: React.FC<ThreatAssessmentProps> = ({ drones }) => {
  ...
  const overallThreatLevel = () => {
    if (threatCounts.CRITICAL > 0) return { level: 'CRITICAL', color: '#ff4444', progress: 100 };
    if (threatCounts.HIGH > 0) return { level: 'HIGH', color: '#ff8800', progress: 80 };
    if (threatCounts.MEDIUM > 0) return { level: 'MEDIUM', color: '#ffaa00', progress: 60 };
    if (threatCounts.LOW > 0) return { level: 'LOW', color: '#00ff41', progress: 30 };
    ...
    <Paper sx={{ p: 2, height: '250px' }}>
      ...
      <Box sx={{ display: 'flex', gap: 2, mb: 2 }}>
        <Box sx={{ flex: 1, textAlign: 'center' }}>
          <Typography variant="h4" sx={{ color: '#00ff41', fontFamily: 'monospace' }}>
            {totalThreats}
          </Typography>
        ...
        <Box sx={{ flex: 1, textAlign: 'center' }}>
          <Typography variant="h4" sx={{ color: '#ff4444', fontFamily: 'monospace' }}>
            {highPriorityThreats}
          </Typography>
          <Typography variant="caption" sx={{ color: '#666' }}>
            HIGH PRIORITY
      ...
      {closestThreat && (
        <Box sx={{ mt: 2, p: 1, backgroundColor: '#1a1a1a', borderRadius: 1 }}>
          ...
          <Typography variant="caption" sx={{ color: threatCounts.CRITICAL > 0 ? '#ff4444' : '#ffaa00' }}>
      ...
      <Box sx={{ mt: 2, display: 'flex', gap: 1, flexWrap: 'wrap' }}>
        {threatCounts.CRITICAL > 0 && (
          <Chip size="small" label={`${threatCounts.CRITICAL} CRITICAL`} color="error" />
        )}
        {threatCounts.HIGH > 0 && (
          <Chip size="small" label={`${threatCounts.HIGH} HIGH`} sx={{ backgroundColor: '#ff8800' }} />
        )}
        {threatCounts.MEDIUM > 0 && (
          <Chip size="small" label={`${threatCounts.MEDIUM} MEDIUM`} color="warning" />
        )}
        {threatCounts.LOW > 0 && (
          <Chip size="small" label={`${threatCounts.LOW} LOW`} color="success" />
        )}
      </Box>
    </Paper>
  );
