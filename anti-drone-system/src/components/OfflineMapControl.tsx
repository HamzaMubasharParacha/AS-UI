...
const OfflineMapControl: React.FC<OfflineMapControlProps> = ({
    ...
    if (isDownloading) {
      const interval = setInterval(() => {
        ...
        if (!progress.isDownloading && progress.total > 0) {
          // Download completed
          setAlertMessage(
            `Download completed! ${progress.total} tiles cached.`
          );
  ...
  const handleDownloadArea = async () => {
    if (!currentBounds) {
      setAlertMessage(
        "No map area selected. Please navigate to the area you want to download."
      );
  ...
  const estimateTileCount = (): number => {
    ...
    for (let z = minZoom; z <= maxZoom; z++) {
      ...
      const y1 = Math.floor(
        ((1 - Math.asinh(Math.tan(latRad1)) / Math.PI) / 2) * n
      );
      const y2 = Math.floor(
        ((1 - Math.asinh(Math.tan(latRad2)) / Math.PI) / 2) * n
      );
  ...
  const handleExportMap = async () => {
    if (!currentBounds) {
      setAlertMessage(
        "No map area selected. Please navigate to the area you want to export."
      );
      setAlertSeverity("warning");
      setTimeout(() => setAlertMessage(null), 5000);
      return;
  ...
  const handleFileSelected = async (
    ...
    try {
      await MapFileExporter.importMapFile(file, (progress, current, total) => {
  ...
  return (
    <Box sx={{ p: 2 }}>
      <Typography
        variant="h6"
        sx={{ mb: 2, color: "#00ff41", fontWeight: "bold" }}
      >
        📡 Offline Map Control
      </Typography>
        ...
        <Card sx={{ mb: 2, backgroundColor: "rgba(255, 165, 0, 0.05)" }}>
          <CardContent>
            ...
            <Typography variant="caption">
              {downloadCurrent} / {downloadTotal} tiles (
      ...
      >
        ...
        <DialogContent>
            ...
            <Typography variant="caption">
              ...
              {getExportSizeEstimate() && (
                <>
                  ...
                  {getExportSizeEstimate()?.estimatedSizeMB.toFixed(1)} MB
                </>
              )}
            </Typography>
          </Alert>
          ...
          <Alert severity="warning">
            <Typography variant="caption">
              This will download map tiles for the current visible area. Large
              areas or high zoom levels may take considerable time and storage.
            </Typography>
          </Alert>
        </DialogContent>
        <DialogActions>
