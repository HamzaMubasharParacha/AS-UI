...
const DroneDetectionPanel: React.FC<DroneDetectionPanelProps> = ({ drones }) => {
  ...
  return (
    <Paper sx={{ p: 2, height: '300px', overflow: 'auto' }}>
      ...
      {drones.length === 0 ? (
        ...
        <List dense>
          {drones.slice(-10).reverse().map((drone, index) => (
                  ...
                  secondary={
                    <Box sx={{ mt: 1 }}>
                      <Typography variant="caption" display="block" sx={{ color: '#aaa' }}>
                        Distance: {drone.distance.toFixed(0)}m | Speed: {drone.speed.toFixed(1)} m/s
                      </Typography>
                      <Typography variant="caption" display="block" sx={{ color: '#aaa' }}>
                        Heading: {drone.heading.toFixed(0)}° | Detected: {formatTime(drone.detected_at)}
                      </Typography>
                      <Typography variant="caption" display="block" sx={{ color: '#aaa' }}>
                        Position: {drone.position[1].toFixed(4)}°N, {drone.position[0].toFixed(4)}°E
                      </Typography>
                    </Box>
                  }
                />
