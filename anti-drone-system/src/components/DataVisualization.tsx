...
const DataVisualization: React.FC<DataVisualizationProps> = ({ drones }) => {
  ...
  const threatData = [
    { name: 'LOW', value: drones.filter(d => d.threat_level === 'LOW').length, color: '#00ff41' },
    { name: 'MEDIUM', value: drones.filter(d => d.threat_level === 'MEDIUM').length, color: '#ffaa00' },
    { name: 'HIGH', value: drones.filter(d => d.threat_level === 'HIGH').length, color: '#ff8800' },
    { name: 'CRITICAL', value: drones.filter(d => d.threat_level === 'CRITICAL').length, color: '#ff4444' },
  ];

  // Prepare data for distance distribution
  const distanceData = [
    { range: '0-500m', count: drones.filter(d => d.distance <= 500).length },
    { range: '500-1000m', count: drones.filter(d => d.distance > 500 && d.distance <= 1000).length },
    { range: '1000-2000m', count: drones.filter(d => d.distance > 1000 && d.distance <= 2000).length },
  ...
  const timelineData = drones.slice(-10).map((drone, index) => ({
    ...
    threat: drone.threat_level === 'CRITICAL' ? 4 : drone.threat_level === 'HIGH' ? 3 : drone.threat_level === 'MEDIUM' ? 2 : 1,
  }));
